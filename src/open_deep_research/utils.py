...
async def select_and_execute_search(search_api: str, query_list: list[str], params_to_pass: dict) -> str:
    ...
    if search_api == "tavily":
        # Call tavily_search_async directly so user config params (max_results, topic) are respected
        search_results = await tavily_search_async(query_list, include_raw_content=True, **params_to_pass)
        return deduplicate_and_format_sources(search_results, max_tokens_per_source=4000, include_raw_content=True)
